...
class Rocket {
    constructor(scene) {
        ...
        // Stage configurations (scaled for game)
        this.stages = [
    ...
    createRocket() {
        ...
        const boosterGeometry = new THREE.CylinderGeometry(0.05, 0.06, 0.25, 16);
        const booster = new THREE.Mesh(boosterGeometry, metalMaterial);
        booster.position.y = 0.125;
        booster.castShadow = true;
        rocketGroup.add(booster);
        
        // Intertank
        const intertankGeometry = new THREE.CylinderGeometry(0.06, 0.06, 0.03, 16);
        const intertank = new THREE.Mesh(intertankGeometry, metalMaterial);
        intertank.position.y = 0.265;
        intertank.castShadow = true;
        rocketGroup.add(intertank);
        
        // Second stage
        const stage2Geometry = new THREE.CylinderGeometry(0.05, 0.06, 0.2, 16);
        const stage2 = new THREE.Mesh(stage2Geometry, metalMaterial);
        stage2.position.y = 0.395;
        stage2.castShadow = true;
        rocketGroup.add(stage2);
        
        // Third stage
        const stage3Geometry = new THREE.CylinderGeometry(0.04, 0.05, 0.15, 16);
        const stage3 = new THREE.Mesh(stage3Geometry, metalMaterial);
        stage3.position.y = 0.52;
        stage3.castShadow = true;
        rocketGroup.add(stage3);
        
        // Nose cone
        const noseGeometry = new THREE.ConeGeometry(0.04, 0.07, 16);
        const nose = new THREE.Mesh(noseGeometry, noseMaterial);
        nose.position.y = 0.595;
        nose.castShadow = true;
        rocketGroup.add(nose);
        
        // Fins
        for (let i = 0; i < 4; i++) {
            const finGeometry = new THREE.BoxGeometry(0.008, 0.12, 0.015);
        ...
        // Engine nozzle
        const nozzleGeometry = new THREE.CylinderGeometry(0.03, 0.04, 0.05, 16);
        ...
        this.mesh.position.set(0, 6.371 + 0.01, 0); // Start on Earth surface (scaled)
    ...
    createExhaustParticles() {
        ...
        for (let i = 0; i < particleCount * 3; i += 3) {
            positions[i + 1] = -0.015; // Start below rocket
            sizes[i / 3] = Math.random() * 0.005 + 0.002;
        }
    ...
    createEngineFlame() {
        ...
        const flameGeometry = new THREE.ConeGeometry(0.04, 0.08, 16);
        ...
        this.engineFlame.position.y = -0.065;
    ...
    update(delta) {
        ...
        // Update altitude (scaled)
        this.altitude = (this.mesh.position.length() - 6.371) * 1000; // Convert to km
    }
